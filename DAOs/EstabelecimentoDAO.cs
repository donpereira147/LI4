        ...
        public SortedDictionary<int, Estabelecimento> getEstabelecimentos(string refe)
        {
            ...
            using (SqlConnection conn = new SqlConnection("Trusted_Connection=true;server =NOMORESEGFAULTS\\SQLEXPRESS;database=MnhamMnham"))
            {
                ...
                try
                {
                    ...
                    SqlCommand est = new SqlCommand("select id_refeiçao, designaçao, tipo_de_cozinha, descriçao, foto_path from Refeiçao where designaçao=@designacao", conn);
                    ...
                    while (reader.Read())
                    {
                        ...
                        using (SqlConnection con = new SqlConnection("Trusted_Connection=true;server =NOMORESEGFAULTS\\SQLEXPRESS;database=MnhamMnham"))
                        {
                            con.Open();
                        ...
                        using (SqlConnection con = new SqlConnection("Trusted_Connection=true;server =NOMORESEGFAULTS\\SQLEXPRESS;database=MnhamMnham"))
                        {
                            ...
                            SqlCommand ests =
                                new SqlCommand("select Estabelecimento.id_estabelecimento, designaçao, NIF, telefone, rede_social, url_site, classificaçao, morada, codigo_postal from MnhamMnham.dbo.Estabelecimento inner join MnhamMnham.dbo.EstabelecimentoRefeiçao on Estabelecimento.id_estabelecimento = EstabelecimentoRefeiçao.id_estabelecimento  where EstabelecimentoRefeiçao.id_refeiçao=@idRefeicao", con);
                            ...
                            while (reader3.Read())
                            {
                                ...
                                if (!estabelecimentos.ContainsKey(idEstabelecimento))
                                {
                                    ...
                                    r.Add(refeicao);
                                    Estabelecimento estabelecimento = new Estabelecimento(url_site, codigo_postal, descriçao, rede_social, telefone, NIF, classificaçao, r);
